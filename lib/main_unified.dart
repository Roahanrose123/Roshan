...
import 'config.dart';
import 'auth_service_unified.dart';
...
void main() async {
  ...
  if (UnifiedAuthService.isFirebaseSupported) {
    try {
...
class AuthWrapper extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<String?>(
      stream: UnifiedAuthService.authStateChanges(),
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 40),
            Text(
              UnifiedAuthService.isFirebaseSupported 
                ? 'Online mode uses Firebase authentication'
                : 'Online mode simulated for desktop',
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _signInWithGoogle() async {
    ...
    try {
      final success = await UnifiedAuthService.signInWithGoogle();
      if (success && mounted) {
  ...
  Future<void> _signInWithMicrosoft() async {
    setState(() => _isLoading = true);
    try {
      final success = await UnifiedAuthService.signInWithMicrosoft();
      if (success && mounted) {
        // User will be automatically redirected by AuthWrapper
      } else if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          ...
          children: [
            ...
            Text(
              UnifiedAuthService.isFirebaseSupported 
                ...
                : 'Simulated authentication for desktop',
              style: const TextStyle(fontSize: 12, color: Colors.grey),
            ),
          ],
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        foregroundColor: Colors.white,
        actions: [
          if (UnifiedAuthService.currentUser != null) ...[
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 8.0),
              child: Center(
                child: Text(
                  UnifiedAuthService.currentUser!,
